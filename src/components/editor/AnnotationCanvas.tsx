...
export const AnnotationCanvas = memo(function AnnotationCanvas({
  ...
  const getResizeHandles = useCallback(
    (annotation: Annotation): Array<{ id: string; x: number; y: number }> => {
      switch (annotation.type) {
        ...
        case "arrow": {
          ...
          if (
            ...
            handles.push({
              id: "control",
              x: annotation.controlPoints[0].x,
              y: annotation.controlPoints[0].y,
            });
          }
          return handles;
        }
  ...
  const getHandleAtPoint = useCallback(
    (point: Point, annotation: Annotation): string | null => {
        ...
        case "arrow": {
          ...
          if (
            ...
            const t = Math.max(
              ...
              Math.min(
                1,
                ((point.x - annotation.x) * dx +
                  (point.y - annotation.y) * dy) /
                  (length * length),
              ),
            );
  ...
  const drawResizeHandles = useCallback(
    (
      ...
      handles.forEach((handle) => {
        ...
        const hoverSize = isLineOrArrow
          ? LINE_HANDLE_SIZE + 4
          : HANDLE_HOVER_SIZE;
        const size = isHovered || isActive ? hoverSize : baseSize;
        const fillColor = isControl
          ? "#10b981"
          : isActive
            ? "#2563eb"
            : "#3b82f6";
    ...
    (
      ...
      if (isSelected && annotation.type !== "blur") {
        ...
        drawResizeHandles(
          ctx,
          annotation,
          resizingAnnotation === annotation.id ? resizeHandle : null,
        );
      }
    },
    [drawResizeHandles],
  ...
  const applyResize = useCallback(
    (
      ...
      switch (annotation.type) {
        case "circle": {
          const distance = Math.sqrt(
            Math.pow(point.x - startAnnotation.x, 2) +
              Math.pow(point.y - startAnnotation.y, 2),
          );
        ...
        case "rectangle": {
          if (
            ...
            startAnnotation.type !== "rectangle"
          )
            return annotation;
        ...
        case "arrow": {
          ...
          if (handle === "start") {
            ...
            if (
              startAnnotation.type !== "line" &&
              startAnnotation.type !== "arrow"
            )
              return annotation;
        ...
        case "text": {
          if (annotation.type !== "text" || startAnnotation.type !== "text")
            return annotation;
          ...
          if (isRightHandle) {
            scaleX = Math.max(
              0.1,
              (point.x - startAnnotation.x) / startAnnotation.width,
            );
          } else if (isLeftHandle) {
            scaleX = Math.max(
              0.1,
              (startAnnotation.x - point.x) / startAnnotation.width,
            );
          }
          ...
          if (isBottomHandle) {
            scaleY = Math.max(
              0.1,
              (point.y - startAnnotation.y) / startAnnotation.height,
            );
          } else if (isTopHandle) {
            scaleY = Math.max(
              0.1,
              (startAnnotation.y - point.y) / startAnnotation.height,
            );
          }
          ...
          const newFontSize = Math.max(
            8,
            Math.round(startAnnotation.fontSize * scale),
          );
          ...
          if (isLeftHandle) {
            newX =
              startAnnotation.x +
              startAnnotation.width -
              startAnnotation.width * fontScale;
          }
          if (isTopHandle) {
            newY =
              startAnnotation.y +
              startAnnotation.height -
              startAnnotation.height * fontScale;
          }

          const newWidth = Math.max(
            50,
            Math.round(startAnnotation.width * fontScale),
          );
          const newHeight = Math.max(
            20,
            Math.round(startAnnotation.height * fontScale),
          );
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (
        ...
        resizeHandle &&
        resizeStartPoint &&
        resizeStartAnnotationRef.current
      ) {
        ...
        rafRef.current = requestAnimationFrame(() => {
          ...
          if (annotation) {
            ...
            if (annotation.type === "line" || annotation.type === "arrow") {
              (
                updated as typeof annotation & { endX: number; endY: number }
